...
def generate_multiple_sql(nl_query: str, db_schema: str, fk_relationships: list, num_beams: int = 5, num_return_sequences: int = 3):
    """
    Generates multiple candidate SQL queries using a few-shot prompt and beam search.
    """
    prompt = _relationship_aware_prompt(nl_query, db_schema, fk_relationships)
    print("--- Generating MULTIPLE SQLs with TRIFECTA MASTERCLASS prompt ---")
    print(prompt)
    inputs = tokenizer(prompt, return_tensors="pt", padding="longest", truncation=True, max_length=1024)
    ...
    for sql in generated_queries:
        ...
        if cleaned_sql.upper().startswith("SQL:"):
            cleaned_sql = cleaned_sql[4:].strip()
        cleaned_queries.append(cleaned_sql)
